...
def getDirSize(dirName, dirDict):
    ...
    if any("dir" in item for item in dirList):
        ...
        for dir in matching:
            dirPath = ""
            if dirName != "/":
                dirPath += dirName + "/" + dir.split()[-1]
            else:
                dirPath += dirName + dir.split()[-1]

            dirSize += getDirSize(dirPath, dirDict)
        for x in dirList:
            if "dir" not in x:
...
winner = 100000000000000
usedSpace  = dirSizeDict["/"]
availableSpace = 70000000 - usedSpace
neededToRemove = 30000000 - availableSpace
print(f"Need to remove: {neededToRemove}")
dirSizeDict.pop("/")
for dirName in dirSizeDict:
    dirSize = dirSizeDict[dirName]
    print(availableSpace - dirSize)
    if dirSize >= neededToRemove:
        print(dirSize)
        if dirSize < winner:
            print("New Winner!")
            winner = dirSize
